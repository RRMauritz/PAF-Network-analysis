...
from Others.Graph_class import Graph, Edge
...
def write_dot(graph: Graph, f: IO[str], directed=False):
    ...
    for v in graph:
        ...
        if hasattr(v, 'colortext'):
            ...
            options += 'color=' + str(v.colornum % NUM_COLORS + 1) + ', color=' + DEFAULT_COLOR_SCHEME + ','
    ...
    for e in graph.edges:
        ...
        if hasattr(e, 'colortext'):
            ...
            options='penwidth=3,'
            options += 'color=' + str(e.colornum % NUM_COLORS + 1) + ', color=' + DEFAULT_COLOR_SCHEME + ','
        if len(options) > 0:
            options = ' [' + options[:-1] + ']'
        if directed:
